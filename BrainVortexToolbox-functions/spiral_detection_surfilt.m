...
for irow = 1:size(temp1_smooth_phase_sur,1)
    for icol = 1:size(temp1_smooth_phase_sur,2)
        ...
        if nansum(temp1(:))~=0
        temp1_smooth_phase_sur(irow,icol,:) = unwrap(angle(hilbert(temp1(:))));
        end
      ...
      for time_withinpatt = 1:size(absoluteTime{ipatt},2);  % select 1 time point within the duration of the detect spiral pattern
        ...
        for irow = 1:size(Vx_flowmap_norm_phase,1) 
            for icol = 1:size(Vx_flowmap_norm_phase,2) 
                y_template(irow,:) = irow;
                x_template(:,icol) = icol;   
            end
        end
        centerofmass_1patt_vx = round(x_template - centerofmass_1patt(1));
        centerofmass_1patt_vy = round(y_template - centerofmass_1patt(2));
        ...
        expansion_threshold = 1;    
        for d = 2:0.5:50    % gradually growing expansion radius, with increments of 0.5 
...
spiral_filt_nega_real = [];          % Patterns x Time Cell Array containing the maps of a particular spiral
spiral_filt_pos_real = [];           % '' '' ''
Vx_flowmap_norm_phase = [];          % Phase Velocity Vector Fields  
Vy_flowmap_norm_phase = [];          % '' '' ''
spiral_size_nega_real= [];           % Spiral Radii
...
for ipatt = 1:size(absoluteTime,2) % select 1 spiral pattern detected   
      for time_withinpatt = 1:size(absoluteTime{ipatt},2);  % select 1 time point within the duration of the detect spiral pattern
        ...
        % exapansion threshold: stop increment expansion of spiral radius if exceeded
        for d = 2:0.5:50    % gradually growing expansion radius, with increments of 0.1 
...
spiral_cav_neg_real=cell(size(spiral_filt_nega_real,1),size(spiral_filt_nega_real,2));
...
if flagRest == 0 
    ...
    elseif flagTask == 4
        if hemisphere == 1
           ...
           save([folder_name,'/',filename],'spiral_filt_nega_centreONLY_real','spiral_filt_pos_centreONLY_real','spiral_filt_nega_real','spiral_filt_pos_real','spiral_filt_nega_real_95perc_extend','spiral_filt_pos_real_95perc_extend','spiral_filt_nega_real_centreONLY_95perc_extend','spiral_filt_pos_real_centreONLY_95perc_extend','spiral_template_filt','spiral_size_real_95perc_accu','spiral_size_pos_real','spiral_size_nega_real','significant_spiral_frame_kept_ratio','significant_spiral_frame_kept_ratio_extend','expansion_threshold','temp1_compatibility_ratio_pos_nega_real','temp1_compatibility_ratio_pos_nega_sur','temp1_compatibility_ratio_pos_nega_real_avg','temp1_compatibility_ratio_pos_nega_sur_avg','spiral_filt_nega_sur','spiral_filt_pos_sur','-v7.3');
        elseif hemisphere == 2
           folder_name = [main_folder,'/Sample Data/Motor Task/Spiral Detected'];
           cd(folder_name)
